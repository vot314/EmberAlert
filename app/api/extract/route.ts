...
// Live Gemini extraction has been observed to take 8-58s. Give it the longest window
// Vercel Hobby allows; on Pro this can go higher.
...
/** The report manifest, for clients that want the queue without analysing it. */
...
export async function POST(request: Request) {
  ...
  const call = (manifest.reports as ReportRecord[]).find((c) => c.id === callId);
  if (!call) {
  ...
  // than reading the filesystem: public/ files are on the local dev server's disk but
  // NOT on Vercel's serverless function filesystem, whereas the static URL resolves
  // identically in both.
